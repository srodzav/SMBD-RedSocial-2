...
public class Consulta2 extends javax.swing.JFrame {
    ...
    public void muestraDB() {
        ...
        try {
            ...
            String cadena = "(SELECT P.id_persona, P.nombre, P.nombre_red_social, P.imagen_perfil, (SELECT COUNT(*) FROM Amigo WHERE id_persona =  " + persona.split("-")[0] + " OR id_persona_amigo =  " + persona.split("-")[0] + " ) as numero_amigos  FROM Amigo as A " +
                    "INNER JOIN Persona as P on A.id_persona_amigo = P.id_persona " +
                    "WHERE A.id_persona = " + persona.split("-")[0] + " ) " +
                    "UNION " +
                    "(SELECT P.id_persona, P.nombre, P.nombre_red_social, P.imagen_perfil, (SELECT COUNT(*) FROM Amigo WHERE id_persona =  " + persona.split("-")[0] + " OR id_persona_amigo =  " + persona.split("-")[0] + " ) as numero_amigos  FROM Amigo as A " +
                    "INNER JOIN Persona as P on A.id_persona = P.id_persona " +
                    "WHERE A.id_persona_amigo = " + persona.split("-")[0] + ") ";
            ...
            while (rs.next()) {
                ...
                datos[4] = rs.getString("numero_amigos");
                modelo.addRow(datos);
